 ...
 */
export const normalizeLoanSlotClaimYieldAndRepay = (transaction: any): LoanSlotClaimYieldAndRepayResult | null => {
  try {
    if (!transaction.events) return null;

    // Find multiple events that can occur during this operation
    const yieldClaimedEvent = transaction.events.find((event: any) => 
      event.type.includes('YieldClaimed')
    );
    
    const loanRepaidEvent = transaction.events.find((event: any) => 
      event.type.includes('LoanRepaid')
    );
    
    const debtIndexUpdatedEvent = transaction.events.find((event: any) => 
    ...
    return {
      ...
      timestamp: yieldData?.timestamp || repayData?.timestamp || debtData?.timestamp || transaction.timestamp,
